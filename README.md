...
- **Сервисный контроль**: Идемпотентность обычных запусков контролируется на уровне бизнес-логики `AnalysisService`. Если для данного `input_hash` уже существует **успешный** анализ (`status=success`), повторный вызов LLM пропускается. Записи `failed`/`invalid_response` не блокируют повторную попытку — обычный (без `--force`) запуск попробует проанализировать материал снова.
- **Ограничение числа попыток**: `LLM_ANALYSIS_MAX_ATTEMPTS` (по умолчанию `3`) ограничивает число неуспешных попыток без `--force` для одного `input_hash`. После исчерпания лимита `analyze_single_item` возвращает `retries_exhausted` и не вызывает LLM повторно, пока кто-то явно не перезапустит анализ через `--force`. При включенном `--force` вызов LLM выполняется принудительно и создается новая запись, независимо от лимита попыток.
